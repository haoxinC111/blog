...
- 支持 chain of thought 结构化步骤输出。
- 步骤：1) 定义 schema；2) 在 API 调用中提供 schema（`text: { format: { type: "json_schema", strict: true, schema: ... } }`）。
- JSON Schema 最佳实践：键名清晰、创建清晰的标题和描述、用 evals 确定最佳结构。
